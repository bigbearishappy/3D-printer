...
// Extruder offset 锟斤拷锟斤拷锟斤拷偏锟斤拷锟斤拷
#if EXTRUDERS > 1
...
void enquecommand(const char *cmd)
{
  if(buflen < BUFSIZE)
  {
    ...
    strcpy(&(cmdbuffer[bufindw][0]),cmd);//复制cmd数组中的值到cmdbuffer
    SERIAL_ECHO_START;
...
void servo_init()
{
    ...
    servos[0].attach(SERVO0_PIN);//将一个伺服电机对象servo[0]连接到SERVO_PIN引脚的舵机上
  #endif
...
void setup()
{
  ...
  SERIAL_PROTOCOLLNPGM("start");//串口打印start
  SERIAL_ECHO_START;
  ...
  while( MYSERIAL.available() > 0  && buflen < BUFSIZE) {
    ...
    if(serial_char == '\n' || serial_char == '\r' || (serial_char == ':' && comment_mode == false) || serial_count >= (MAX_CMD_SIZE - 1) )
    {
      ...
      if(!comment_mode){
        ...
        if(strchr(cmdbuffer[bufindw], 'N') != NULL)//strchr function:search for 'N' in cmfbuffer[bufindw].if it exists,return the pointer of it,or return NULL
        {
          strchr_pointer = strchr(cmdbuffer[bufindw], 'N');
          gcode_N = (strtol(&cmdbuffer[bufindw][strchr_pointer - cmdbuffer[bufindw] + 1], NULL, 10));//transform the target data to decimal number
          if(gcode_N != gcode_LastN+1 && (strstr_P(cmdbuffer[bufindw], PSTR("M110")) == NULL) ) {	//strstr_P function:find out if PSTR("M110") is in the cmdbuffer[bufindew]
          																							//if so,return the pointer that the PSTR("M110") first exist in cmdbuffer[bufindw],or return NULL.
            SERIAL_ERROR_START;
            SERIAL_ERRORPGM(MSG_ERR_LINE_NO);//print the error to the console
          ...
          {
            byte checksum = 0;//byte data can be + or -,but char just can be +
            byte count = 0;
...
long code_value_long()//根据将字符串转换成长整形数
{
...
/********************************************************************************************************************
name:		code_seen()
function:	try to find a code in a string,return true if it is found,else false
			[in]	-	code
			[out]	-	true or false
********************************************************************************************************************/
...
function:	Calculate the steps/s^2 acceleration rates, based on the mm/s^s
			[in]	-	void
			[out]	-	void
***************************************************************************************************/
...
/***************************************************************************************************
name:		homeaxis()
function:	make the specify axis to move home
			[in]	-	void
			[out]	-	void
***************************************************************************************************/
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 4: // G4 dwell							//wait for n milliseconds
      ...
      previous_millis_cmd = millis();
      while(millis()  < codenum ){				//when the printer wait,the extruder's temperature can be control.same with lcd
        manage_heater();
        manage_inactivity();
        lcd_update();
      }
      break;
      #ifdef FWRETRACT
    case 10: // G10 retract(回收)					//extruder's plastic retract
      if(!retracted)
      {
      ...
      break;
    case 11: // G11 retract_recover					//recover when the extruder's plastic retract
      if(retracted)
      {
        destination[X_AXIS]=current_position[X_AXIS];
        destination[Y_AXIS]=current_position[Y_AXIS];
    ...
    case 30: // G30 				//Single Z Probe,check the bed on the current position
        {
            engage_z_probe(); // Engage Z Servo endstop if available
  ...
  else if(code_seen('M'))
  {
    ...
    #endif //PIDTEMP
    #ifdef PIDTEMPBED
    case 304: // M304							//set the value of P I D
      {
...
/***************************************************************************************
name:		controllerFan()
function:	control the fan according to the time
			[in]	-	void
			[out]	-	void
***************************************************************************************/
void controllerFan()
